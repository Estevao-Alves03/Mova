...
- Perfil do paciente (`GET /patients/{id}`, `patients.service.get_patient`): mesmo escopo de `scope_patients` (fora dele = 404 uniforme) e schema por papel (`response_model=None`): `PatientDetailBasic` para a recepção, `PatientDetailClinical` para admin e nutricionista. A situação sai de `_summarize`, compartilhada com a lista.
