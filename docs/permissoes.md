...
* **Perfil do paciente** (`GET /patients/{id}`, tela `/app/patients/:id`): a API responde com o schema do papel. **Recepção**: só cadastro (nome, nascimento, sexo, telefone, e-mail, data de cadastro), responsável pelo atendimento (nome e CRN), unidade e situação; **nenhuma** consulta, meta, anamnese, avaliação, evolução ou anotação (as chaves clínicas nem existem na resposta). **Admin** e **Nutricionista responsável** recebem também o objetivo e a observação inicial (da 1ª consulta), a última e a próxima consulta e a linha do tempo de consultas (sem canceladas). Nutricionista pedindo paciente de outro profissional, paciente de outra clínica, excluído ou inexistente: **404** idêntico (não revela que existe). A unidade não pertence ao cadastro: vem da próxima consulta ativa (ou, sem ela, da mais recente). O modelo de metas numéricas (peso, % de gordura etc.) **ainda não está definido**: a seção Metas mostra apenas o objetivo informado no cadastro.
...
* [ ] Perfil do paciente *(teste escrito, ainda não executado contra o Postgres)*: recepção recebe só cadastro, responsável e unidade (sem `goal`, `initial_notes`, `timeline` nem próxima consulta); nutricionista e admin recebem os dados clínicos; nutricionista com paciente de outro profissional recebe o mesmo 404 de um id inexistente; outra clínica e excluído são 404; sem token `401`, paciente e sem membership `403` (`test_patient_detail_api.py`).
